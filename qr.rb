require 'rqrcode'
require 'rqrcode_png'

puts 'What kind of QR code would you like to generate?'
puts
puts '1. Open a URL'
puts '2. Join a wifi network'
puts '3. Send a text message'
puts

kind_of_code = gets.chomp
puts

if kind_of_code.include?('1')
  puts 'What is the URL you would like to encode?'
  puts

  text_to_encode = gets.chomp
  puts
elsif kind_of_code.include?('2')
  puts 'What is the name of the wifi network?'
  puts

  network_name = gets.chomp
  puts

  puts 'What is the password?'
  puts

  network_password = gets.chomp
  puts

  text_to_encode = "WIFI:T:WPA;S:#{network_name};P:#{network_password};;"
elsif kind_of_code.include?('3')
  puts 'What is the phone number you want the code to send a text message to?'
  puts

  phone_number = gets.chomp
  puts

  puts 'What do you want to populate the message with?'
  puts

  message_body = gets.chomp
  puts

  text_to_encode = "SMSTO:#{phone_number}:#{message_body}"
else
  puts "Didn't recognize that selection. Please try again."
  puts
  exit 1
end

puts 'What would you like to call the PNG file? (e.g., my_qr_code)'
puts

filename = gets.chomp
puts

qrcode = RQRCode::QRCode.new(text_to_encode)

# Create an instance of QRCode with size and border options
png = qrcode.as_png(
  resize_gte_to: false,
  resize_exactly_to: true,
  fill: 'white',
  color: 'black',
  size: 300,
  border_modules: 4
)

# Create an instance of RQRCode::QRCode for the image
qr = RQRCode::QRCode.new(text_to_encode)

# Load an image that you want to embed in the QR code (e.g., 'image.png')
image = ChunkyPNG::Image.from_file('image.png')

# Create a QR code with the image embedded
qr_with_image = qr.as_png(
  resize_gte_to: false,
  resize_exactly_to: true,
  fill: 'white',
  color: 'black',
  size: 300,
  border_modules: 4,
  file: image
)

IO.binwrite("#{filename}.png", qr_with_image.to_s)
puts "QR code saved as #{filename}.png"
